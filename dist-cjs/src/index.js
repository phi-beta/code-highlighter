...
exports.unregisterLanguage = unregisterLanguage;
exports.registerOutputHandler = registerOutputHandler;
exports.getOutputHandler = getOutputHandler;
exports.listOutputHandlers = listOutputHandlers;
exports.highlight = highlight;
exports.exportTokens = exportTokens;
exports.exportTokensAsJson = exportTokensAsJson;
/**
...
try {
    ...
    if (process.pkg) {
        ...
        ansiDefault = embedded_js_1.ANSI_DEFAULT_CONFIG;
        htmlDefault = embedded_js_1.HTML_DEFAULT_CONFIG;
        htmlTheme = embedded_js_1.HTML_THEME;
        ansiTheme = embedded_js_1.ANSI_THEME;
    }
    else {
        // Normal execution, try to load from files
        ansiDefault = JSON.parse(node_fs_1.default.readFileSync((0, assets_js_1.getAssetPath)('./handlers/ansi.config.json'), 'utf-8'));
        htmlDefault = JSON.parse(node_fs_1.default.readFileSync((0, assets_js_1.getAssetPath)('./handlers/html.config.json'), 'utf-8'));
        htmlTheme = JSON.parse(node_fs_1.default.readFileSync((0, assets_js_1.getAssetPath)('./themes/html.theme.json'), 'utf-8'));
        ansiTheme = JSON.parse(node_fs_1.default.readFileSync((0, assets_js_1.getAssetPath)('./themes/ansi.theme.json'), 'utf-8'));
    }
}
catch (error) {
...
const htmlBaseTheme = htmlTheme;
const ansiBaseTheme = ansiTheme;
const languages = new Map();
function registerLanguage(name, tokenizer) { languages.set(name.toLowerCase(), tokenizer); }
// Safer idempotent variant (internal use): only replace if different reference
function ensureLanguage(name, tokenizer) {
    const k = name.toLowerCase();
    const existing = languages.get(k);
    if (!existing || existing !== tokenizer)
        languages.set(k, tokenizer);
}
function getLanguage(name) { return languages.get(name.toLowerCase()); }
function listLanguages() { return [...languages.keys()].sort(); }
// Test/maintenance helper: allow removal in case a test needs a clean slate.
function unregisterLanguage(name) { languages.delete(name.toLowerCase()); }
// ANSI helpers (theme already stores ANSI escape codes in color field)
function applyAnsi(style, text) {
    let prefix = '';
    if (style.color)
        prefix += style.color; // expected to be an ANSI escape sequence
    if (style.fontStyle) {
        const map = { bold: '\u001b[1m', italic: '\u001b[3m', underline: '\u001b[4m', dim: '\u001b[2m' };
        if (map[style.fontStyle])
            prefix += map[style.fontStyle];
    }
    if (!prefix)
        return text;
...
function themeToCss(theme) {
    ...
    for (const [type, style] of Object.entries(theme)) {
        ...
        if (css.length)
            parts.push(`.tok-${type}{${css.join(';')}}`);
    }
    return parts.join('');
}
// Lazily initialized map to avoid any temporal dead zone or cyclic-import timing issues.
let _outputHandlers;
function ensureOutputHandlers() { if (!_outputHandlers)
    _outputHandlers = new Map(); return _outputHandlers; }
function registerOutputHandler(handler) { ensureOutputHandlers().set(handler.id, handler); }
function getOutputHandler(id) { return ensureOutputHandlers().get(id); }
function listOutputHandlers() { return [...ensureOutputHandlers().keys()].sort(); }
...
function ensureBuiltInHandlers() {
    ...
    registerOutputHandler({
        ...
        render(tokens, theme, ctx) {
            ...
            if (block)
                return `<pre class=\"ch-pre\"><code class=\"ch-code lang-${language}\">${body}</code></pre>`;
            return body;
        }
    });
}
function highlight(code, options = {}) {
    ...
    if (langName === 'markdown' || langName === 'md') {
        ...
        tokens = tokens.map(tok => {
            ...
            if (/^ {0,3}>( |$)/.test(line))
                return { ...tok, type: 'blockquote' };
            if (/^ {0,3}(?:---+|\*\*\*+|___+)[ \t]*$/.test(line))
                return { ...tok, type: 'rule' };
            if (/^ {0,3}[-*+] +(?!\[[ xX]\]).*\S/.test(line))
                return { ...tok, type: 'list-bullet' };
            if (/^ {0,3}\d+\. +\S/.test(line))
                return { ...tok, type: 'list-enum' };
            if (/^#+\s+/.test(line))
                return { ...tok, type: 'heading' };
            if (/^```/.test(line)) {
                // classify as fence start or end depending on if more backticks only
                if (/^```\s*[^`]*$/.test(line) && !/^```+$/.test(line.trim()))
                    return { ...tok, type: 'code-fence-start' };
                if (/^```+$/.test(line.trim()))
                    return { ...tok, type: 'code-fence-end' };
            }
            if (/^~~.+~~$/.test(line))
                return { ...tok, type: 'strike' };
            if (/^!\[[^\]]*\]\([^\)]+\)$/.test(line))
                return { ...tok, type: 'image' };
            return tok;
        });
                    ...
                    if (lang && getLanguage(lang)) {
                        try {
                            ...
                            for (const nt of nested)
                                final.push({ type: nt.type, value: nt.value });
                        }
                        catch {
        ...
        for (const tok of tokens) {
            if (tok.type !== 'identifier' && tok.type !== 'TEXT'.toLowerCase() && tok.type !== 'code-fence-content') {
                out.push(tok);
                continue;
            }
            ...
            while ((m = pattern.exec(value))) {
                if (m.index > lastIndex)
                    out.push({ type: 'identifier', value: value.slice(lastIndex, m.index) });
                const seg = m[0];
                if (seg.startsWith('!['))
                    out.push({ type: 'image', value: seg });
                else if (seg.startsWith('**'))
                    out.push({ type: 'strong', value: seg });
                else if (seg.startsWith('*'))
                    out.push({ type: 'emphasis', value: seg });
                else if (seg.startsWith('`'))
                    out.push({ type: 'code-inline', value: seg });
                else if (seg.startsWith('<'))
                    out.push({ type: 'autolink', value: seg });
                else if (seg.startsWith('[^'))
                    out.push({ type: 'footnote', value: seg });
                else if (seg.includes(']:'))
                    out.push({ type: 'link-definition', value: seg });
                else if (seg.includes(']['))
                    out.push({ type: 'link-reference', value: seg });
                else if (seg.startsWith('['))
                    out.push({ type: 'link', value: seg });
                else
                    out.push({ type: 'identifier', value: seg });
                lastIndex = m.index + seg.length;
            }
            if (lastIndex < value.length)
                out.push({ type: 'identifier', value: value.slice(lastIndex) });
        }
        ...
        for (let i = 0; i < out.length; i++) {
            ...
            if (tok.type === 'identifier' && tok.value.endsWith('  ') &&
                ...
                if (content)
                    processedOut.push({ type: 'identifier', value: content });
        ...
        tokens = tokens.map(t => {
            ...
            if (/^ {0,3}[-*+] +(?!\[[ xX]\]).*\S/.test(v))
                return { ...t, type: 'list-bullet' };
            if (/^ {0,3}\d+\. +\S/.test(v))
                return { ...t, type: 'list-enum' };
            if (/^#+\s+/.test(v))
    ...
    if (!handler)
        throw new Error('No output handler available');
    ...
    if (options.bracketNesting !== false) {
        ...
        tokens = tokens.map(tok => {
            if (tok.value.length !== 1)
                return tok;
            const ch = tok.value;
            if (!(ch in openToClose) && !closes.has(ch))
                return tok;
    ...
    if (outputId === 'html') {
        if (options.block !== undefined)
            legacy.block = options.block;
        if (options.fullDocument !== undefined)
            legacy.fullDocument = options.fullDocument;
        if (options.title !== undefined)
            legacy.title = options.title;
    }
 ...
 */
function exportTokens(code, language = 'javascript') {
 ...
 */
function exportTokensAsJson(code, language = 'javascript', pretty = true) {
    const analysis = exportTokens(code, language);
    return pretty ? JSON.stringify(analysis, null, 2) : JSON.stringify(analysis);
}
exports.default = { highlight, exportTokens, exportTokensAsJson, registerLanguage, unregisterLanguage, listLanguages, registerOutputHandler, listOutputHandlers };
