 ...
 */
export function validateTokenAnalysis(analysis: any): analysis is TokenAnalysis {
 ...
 */
export function getTokenAnalysisSchema(): object {
  try {
    const schemaPath = path.join(__dirname, '../schemas/token-analysis.json');
    const schemaContent = fs.readFileSync(schemaPath, 'utf-8');
    return JSON.parse(schemaContent);
  } catch (error) {
    throw new Error(`Failed to load token analysis schema: ${(error as Error).message}`);
  }
}
 ...
 */
export function createSchemaValidator(): (data: any) => boolean {
  try {
