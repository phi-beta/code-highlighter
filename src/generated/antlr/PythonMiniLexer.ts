/** Stub PythonMiniLexer (placeholder until real ANTLR generated). */
export class PythonMiniLexer {
  ...
  private lex(code:string){
    const kw=/^(def|return|if|elif|else|for|while|import|from|as|class|try|except|finally|with|yield|lambda|pass|break|continue|and|or|not|in|is|None|True|False)\b/;
    while(code.length){
      if(/^#/.test(code)){ const m=code.match(/^#.*?(\r?\n|$)/)!; this.parts.push({type:7,text:m[0]}); code=code.slice(m[0].length); continue; }
      if(/^"""/.test(code)||/^'''/.test(code)){ const q=code.startsWith('"""')?'"""':'\'\'\''; const i=code.indexOf(q,3); const seg=i>=0?code.slice(0,i+3):code; this.parts.push({type:2,text:seg}); code=code.slice(seg.length); continue; }
